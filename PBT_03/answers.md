...
Với Hộp 1 (`content-box`): Đây là cơ chế tính kích thước mặc định của trình duyệt. Thuộc tính `width: 300px` chỉ áp dụng riêng cho vùng chứa nội dung (Content). Chiều rộng thực tế hiển thị trên màn hình sẽ cộng thêm phần đệm và phần viền:
  Chiều rộng thực tế = Width + Padding L/R + Border L/R
  Chiều rộng thực tế = 300px + (20px x 2) + (5px x 2) = 350px
...
Với Hộp 2 (`border-box`): Trình duyệt khóa chặt chiều rộng thực tế của hộp bằng đúng giá trị khai báo (`width: 300px`). Để làm được điều này, trình duyệt tự động ép/co nhỏ vùng chứa nội dung (Content) bên trong lại:
  Content Width = Width khai báo - Padding L/R - Border L/R
...
Cột trái: rộng = 250px + (15px x 2 padding) = 280px
Cột giữa: rộng = 500px + (20px x 2  padding) = 540px
Cột phải: rộng = 250px + (15px x 2 padding) = 280px

Tổng chiều rộng thực tế = 280px + 540px + 280px = 1100px

> Kết quả: Vì tổng thực tế (1100px) lớn hơn kích thước của Container (1000px), cột thứ ba (ads) sẽ không đủ chỗ trống để hiển thị trên cùng một hàng và ngay lập tức bị đẩy rơi xuống dòng dưới gây vỡ layout.

2. Khi sử dụng `border-box`:
Cả 3 cột tự động co vùng content bên trong lại để giữ nguyên kích thước bề ngoài đúng như khai báo 250px + 500px + 250px = 1000px. Do đó, layout 3 cột xếp thẳng hàng hoàn hảo và khít vừa vặn trong Container.
**Bài B3 — Specificity Battle**
1. Bảng liệt kê 10 Rules và Điểm số Specificity (a, b, c)

Quy ước tính điểm của Trình duyệt:
a: Số lượng bộ chọn ID
b: Số lượng bộ chọn Class, Thuộc tính (Attribute), Giả lập (Pseudo-class)
c: Số lượng bộ chọn Thẻ (Element), Phần tử giả lập (Pseudo-element)

| STT | Cú pháp CSS Selector | Điểm Score (a, b, c) | Màu sắc định nghĩa |
| :--- | :--- | :--- | :--- |
| 1 | `*` | (0, 0, 0) | Xám (`#94a3b8`) |
| 2 | `p` | (0, 0, 1) | Tím (`#7c3aed`) |
| 3 | `div div p` | (0, 0, 3) | Xanh lá (`#059669`) |
| 4 | `.text` | (0, 1, 0) | Cam (`#ea580c`) |
| 5 | `p.text` | (0, 1, 1) | Hồng (`#db2777`) |
| 6 | `.text.highlight` | (0, 2, 0) | Xanh dương (`#0284c7`) |
| 7 | `.container p.text.highlight` | (0, 3, 1) | Nâu (`#b45309`) |
| 8 | `#demo` | (1, 0, 0) | Đỏ nhạt (`#e11d48`) |
| 9 | `.main-wrapper #demo.highlight` | (1, 1, 1) | Đen (`#111827`) |
| 10 | `#app-root #demo` | (2, 0, 0) | Xanh Royal (`#2563eb`) |
2. Câu hỏi phân tích lý thuyết

1.
Kết quả hiển thị: Dòng chữ "Hello World" sẽ hiển thị Màu xanh Royal (`#2563eb`).
Giải thích: Trình duyệt thực hiện so sánh điểm số Specificity theo thứ tự từ trái qua phải (So sánh cột `a` trước, nếu bằng nhau mới xét cột `b`, rồi đến cột `c`). 
  Rule số 10 (`#app-root #demo`) sở hữu tới **2 bộ chọn ID**, đạt điểm số `(2, 0, 0)`. Điểm số ở cột `a = 2` này lớn hơn tất cả các rule còn lại (Rule 8 và 9 chỉ có `a = 1`, các rule từ 1 đến 7 có `a = 0`). Do đó, Rule 10 chiến thắng tuyệt đối bất chấp các luật khác viết thế nào.
2.
Kết quả: KHÔNG THAY ĐỔI. Dòng chữ vẫn giữ nguyên Màu xanh Royal.
Giải thích: Quy tắc dòng chảy viết sau đè viết trước (Cascade) chỉ được kích hoạt khi và chỉ khi các CSS rules đang tranh chấp có điểm số Specificity bằng nhau chằn chặn. 
  Trong bài tập này, do chúng ta thiết kế 10 bộ chọn có 10 điểm số phân cấp hoàn toàn khác nhau từ thấp đến cao, trình duyệt sẽ luôn luôn ưu tiên áp dụng rule có điểm Specificity cao nhất (`2, 0, 0`), bất kể bạn đảo nó lên đầu file hay giấu ở giữa file CSS.
**PHẦN C — DEBUG & SUY LUẬN**
**CÂU C2 — CASCADE PUZZLE**
Dự đoán:
1. "Sản phẩm A" (h2): font-size = 20px | color = Màu xanh lá (green)
    Về font-size:
...
3. "Sản phẩm B" (h2): font-size = 20px | color = Màu xanh dương (blue)
    Về font-size:
